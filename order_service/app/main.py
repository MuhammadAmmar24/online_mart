...
from app.kafka.consumers.user_response_consumer import consume_user_response
from app.kafka.consumers.product_response_consumer import consume_product_response
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    task = asyncio.create_task(consume_user_response(
        settings.KAFKA_USER_RESPONSE_TOPIC, 
        settings.BOOTSTRAP_SERVER, 
        settings.KAFKA_CONSUMER_GROUP_ID_FOR_USER_RESPONSE
        ))
    asyncio.create_task(consume_product_response(
        settings.KAFKA_PRODUCT_RESPONSE_TOPIC, 
        settings.BOOTSTRAP_SERVER, 
        settings.KAFKA_CONSUMER_GROUP_ID_FOR_PRODUCT_RESPONSE
        ))
    asyncio.create_task(consume_inventory_response(
...
async def call_add_order(
    ...
    await produce_message_to_user(order_model, producer)
...
async def call_update_order(
    ...
    await produce_message_to_inventory(existing_order)
    return {
