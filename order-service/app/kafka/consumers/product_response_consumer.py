...
async def process_product_response(protobuf_response, validation):
    logger.info(f"Processing product response for order: {protobuf_response}")
    ...
    try:
        if validation == 'validated':
            ...
            logger.info(f"Order ID {protobuf_response.id} marked as valid by product service")
            logger.info(f"New Order: {order}")

            logger.info(f"Producing inventory request...")
            await produce_message_to_inventory(order)
            
        else:
            logger.info(f"Order ID {protobuf_response.id} marked as invalid: {validation} by product service")
...
async def consume_product_response(topic, bootstrap_servers, group_id):
    retries = 0
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            logger.info("Product Response Consumer created, attempting to start...")
            await consumer.start()
            logger.info("Product Response Consumer started successfully")
            break
    ...
    try:
        async for msg in consumer:
            ...
            logger.info(f"Consumed Product Response Data: {protobuf_response}")

            await process_product_response(protobuf_response, validation)
